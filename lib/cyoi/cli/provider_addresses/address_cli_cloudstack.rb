...
class Cyoi::Cli::Addresses::AddressCliCloudstack
  ...
  # check if we already have an IP, if so use that if not, get a new one!
  protected
  def provision_address
    ips = provider_client.publicips
    if ips.count < 2
      print "Acquiring a public IP address for port nat... "
      attributes["ip"] = provider_client.provision_public_ip_address
    else
      ip = ips.select{|ip| ip['issourcenat'] == false }
      if ip.count == 1
         ip = ip[0]
      elsif ip.count > 1
        hl.choose do |menu|
          menu.prompt = "Choose an ip for port nat: "
          ips.each do |ip|
            unless ip['issourcenat'] == true
              menu.choice(ip['ipaddress']) { ip }
            end
          end
        end
      end
      # attributes["ip"] = ips[0]['ipaddress']
    end
    attributes["ip"] = ip['ipaddress']
  end
  ...
  # subnet is the cidr for a network in the vpc
  def select_subnet_for_vpc(vpc)
    nets = provider_client.networks
    subnets = nets.select {|net|  net['vpcid'] == vpc['id']}
  ...
  # cloudstack gives us an IP so this will be optional!
...
Cyoi::Cli::Address.register_address_cli("cloudstack", Cyoi::Cli::Addresses::AddressCliCloudstack)
